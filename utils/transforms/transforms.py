    ...
    def parse_resize(image, target, size_image, size_label, ori_size, ignore_x):
        ...
        if target is None or isinstance(target, str):
            ...
            if 'segmentation_mask_liquid' in target.keys():
                target['segmentation_mask_liquid'] = F.resize(target['segmentation_mask_liquid'],
                                                              size_label, interpolation=Image.NEAREST)
        else:
...
class ToTensor(object):
    ...
    def __call__(self, image, target=None):
        ...
        if isinstance(target, dict):
            if 'color_residual' in target.keys():
                target['color_residual'] = self._pil_to_tensor(target['color_residual'])
        ...
        if pic is None or isinstance(pic, str):
            ...
            for k in pic.keys():
                if k in ['keypoints', 'offsets']:
                    ...
                    pic[k] = torch.as_tensor(np.asarray(pic[k]).copy(), dtype=torch.uint8)
                elif k == 'segmentation_mask_liquid':
                    pic[k] = torch.as_tensor(np.asarray(pic[k]).copy(), dtype=torch.uint8)
...
class Normalize(object):
    ...
    def __call__(self, image, target=None):
        ...
        if isinstance(target, dict):
            if 'color_residual' in target.keys():
                target['color_residual'] = F.normalize(target['color_residual'], mean=0, std=self.std)
...
class ColorJitter(object):
    ...
    def __call__(self, image, target):
        ...
        # # should not use color Jitter now for the colornet
        # for fn_id in fn_idx:
        ...
        if isinstance(target, dict):
            if 'segmentation_mask_liquid' in target.keys():
                ...
                # generate color residual
                img_array = np.array(image)
                mask_array = np.array(target['segmentation_mask_liquid'])
                
                color_mean = np.mean(img_array[mask_array == 255], axis=0)
                color_residual = np.zeros_like(img_array)
                
                color_residual[..., 0] = np.where(mask_array == 255, color_mean[0] - img_array[..., 0], 0)
                color_residual[..., 1] = np.where(mask_array == 255, color_mean[1] - img_array[..., 1], 0)
                color_residual[..., 2] = np.where(mask_array == 255, color_mean[2] - img_array[..., 2], 0)

                color_residual = Image.fromarray(color_residual)
                target.update({'color_residual': color_residual})
        return image, target
