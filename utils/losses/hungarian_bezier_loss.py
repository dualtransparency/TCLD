...
class HungarianBezierLoss(WeightedLoss):
    def __init__(self, curve_weight=1, label_weight=0.1, seg_weight=0.75, color_weight=1, alpha=0.8, 
        ...
        # self.color_loss = torch.nn.SmoothL1Loss()
        self.color_loss = torch.nn.MSELoss()
        # self.hist_block = RGBuvHistBlock(insz=512)
    ...
    def forward(self, inputs, targets, net):
        ...
        output_color_residuals = outputs['color_residual']
        target_color_residuals = torch.stack([target['color_residual'] for target in targets], dim=0)
        loss_color = self.color_loss(output_color_residuals, target_color_residuals)
        # output_color_residuals_hist = self.hist_block(output_color_residuals)
        # target_color_residuals_hist = self.hist_block(target_color_residuals)

        # loss_color = (1/np.sqrt(2.0) * (torch.sqrt(torch.sum(torch.pow(torch.sqrt(target_color_residuals_hist) - torch.sqrt(output_color_residuals_hist), 2)))) / output_color_residuals_hist.shape[0])
        ...
        # for loss ablation
        loss_curve = self.point_loss(output_curves, target_keypoints)
        ...
        loss = self.label_weight * loss_label + self.curve_weight * loss_curve + self.seg_weight * loss_seg + self.color_weight * loss_color
        # for ablation exp, no color loss
        ...
        return loss, {'training loss': loss, 'loss label': loss_label, 'loss curve': loss_curve,
                      'loss seg': loss_seg, 'loss_color': loss_color,
